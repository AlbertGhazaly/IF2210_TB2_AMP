...
public class saveloadJSON implements Entity{
    ...
    public void save(String folder) throws IOException {
        ...
        this.savePlayer(gameStatus.getObjek().getPlayer1(), folder+ "/player1.json");
        this.savePlayer(gameStatus.getObjek().getPlayer2(),folder + "/player2.json");
        this.saveToko(folder + "/gamestate.json");
    }

    @Override
    public void load(String folder) throws IOException {
        if (folder.length() != 0) {
            this.loadPlayer(gameStatus.getObjek().getPlayer1(),folder + "/player1.json");
            this.loadPlayer(gameStatus.getObjek().getPlayer2(),folder + "/player2.json");
            this.loadToko(folder + "/gamestate.json");
        }
    }
}
